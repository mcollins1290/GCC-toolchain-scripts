...
#
# v0.0.19:
#   - Fix: trust store assertion was too brittle (whitespace, leading "*", stderr,
#          and occasional missing CAfile/CApath lines depending on curl verbosity/build).
#     New behavior:
#       * Capture full curl -vI output (2>&1), normalize CRLF, parse CAfile/CApath robustly.
#       * If curl REPORTS CAfile/CApath, enforce they match Debian/RPi defaults.
#       * If curl does NOT report them, WARN (do not fail) but still require TLS succeeds.
#       * Always print the observed values to make failures self-explaining.
#   - Minor hardening: guard readelf/ldd of integrated curl when missing.
#   - Add A_PLUS=1 "super suite" switch (one-go invocation) and surface it in report/usage.
#   - Fix: OpenSSL https check inside SSH uses derived url variable for consistency.
# ==============================================================================
