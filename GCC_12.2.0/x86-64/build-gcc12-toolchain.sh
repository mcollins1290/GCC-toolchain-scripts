...
# Triplet & paths
# Use a canonical native triplet for a clean bootstrap.
...
########################################################################
# Step 1: Install Linux headers into sysroot
########################################################################
...
build_gcc_stage1() {
    ...
    # Minimal, libc-free cross-ish GCC (LFS-style pass 1), but we only
    # build the pieces we actually need: gcc + target libgcc.
    ...
    # IMPORTANT: do not run plain "make" here; that tries to configure
    # stage2 libs and hits GCC_NO_EXECUTABLES+CET issues.
...
########################################################################
# Step 4: Glibc headers + start files
########################################################################
...
build_glibc_headers_startfiles() {
    ...
    # For cross-style glibc we configure with --prefix=/usr, then install into SYSROOT via DESTDIR
...
########################################################################
# Step 6: GCC final, bootstrapped (stage1/2/3, object comparison)
########################################################################

build_gcc_bootstrap() {
    local bdir="${BUILD_DIR}/gcc-${GCC_VER}-bootstrap"
    rm -rf "${bdir}"
    mkdir -p "${bdir}"
    cd "${bdir}"
    ...
    # Use system compilers as initial host compiler.
    unset CC CXX AR RANLIB LD
    export CC=gcc
    export CXX=g++

    log_step "gcc-bootstrap-configure" \
        "${SRC_DIR}/gcc-${GCC_VER}/configure" \
    ...
    # This does:
    #  - stage1 GCC built by CC/CXX (system gcc/g++)
    #  - stage2 GCC built by stage1
    #  - stage3 GCC built by stage2
    #  - compare stage2 vs stage3 objects
    log_step "gcc-bootstrap-make" make -j"${NPROC}" bootstrap
    log_step "gcc-bootstrap-install" make install
}
...
#include <stdio.h>
int main(void) {
    printf("Hello from custom GCC toolchain!\n");
    return 0;
}
EOF
    ...
    echo
    echo "================================================================"
    echo "==> Building test program with ${TARGET}-gcc"
    echo "================================================================"
    "${TARGET}-gcc" -o "${tmpdir}/hello" "${tmpdir}/hello.c"

    echo "Test binary linked. Running it with host loader:"
    "${tmpdir}/hello" || true

    echo
    echo "================================================================"
    echo "Bootstrapped toolchain appears to be usable."
    echo "Prefix : ${PREFIX}"
